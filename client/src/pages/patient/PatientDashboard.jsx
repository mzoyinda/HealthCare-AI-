    ...
    const fetchData = async () => {
      try {
        ...
        const myAppointments = appointments.filter(appointment =>
          appointment.patient?._id === currentUserId || appointment.patient === currentUserId
        );
        setRecentAppointments(myAppointments.slice(0, 3));
        const analyses = analysesResponse.analyses || [];
        const myAnalyses = analyses.filter(a =>
          a.patient?._id === currentUserId || a.patient === currentUserId
        );
        setRecentAnalyses(myAnalyses.slice(0, 3));
